...
public interface BoardDB_Oracle {
	...
	String SQL_BOARD_SEARCH_LIST_TITLE = "SELECT * FROM (SELECT * FROM BOARD WHERE CLUB_ID = ?) WHERE UPPER(TITLE) LIKE UPPER(?)";
	String SQL_BOARD_SEARCH_LIST_CONTENT = "SELECT * FROM (SELECT * FROM BOARD WHERE CLUB_ID = ?) WHERE UPPER(CONTENT) LIKE UPPER(?)";
